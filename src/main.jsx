      ...
      return (
        <div style={styles.errScreen}>
          ...
          {import.meta.env.DEV && this.state.error && (
            <pre style={styles.errPre}>{String(this.state.error?.message || this.state.error)}</pre>
          )}
        </div>
      );
    }
...
if (!window.__LICENSE_GUARD_ACTIVE) {
  document.body.innerHTML =
    "<h1 style='color:#f55;font-family:sans-serif;text-align:center;margin-top:15vh'>Lisans Guard Yok</h1>";
...
function logPerfMarks() {
  try {
    ...
    const map = {};
    marks.forEach((m) => {
      map[m.name] = m.startTime.toFixed(1) + 'ms';
    });
    if (isDev) {
      // eslint-disable-next-line no-console
      console.table(map);
